...
async def test_auth(test_client):
    ...
    response = await test_client.post('/auth/register', 
                        json={'email': test_email, 'password': text_passwd})
    ...
    response_email_dublicate = await test_client.post('/auth/register', 
                        json={'email': test_email, 'password': text_passwd})
    assert response_email_dublicate.status_code == 409
    
    #проверяем логин
    #теперь отправляем не в json, а в формате OAuth2
    response_for_login = await test_client.post('/auth/login', 
                        data={'email': test_email, 'password': text_passwd})
    ...
    response_login_with_wrong_pswd = await test_client.post('/auth/login', 
                        data={'email': test_email, 'password': 'auaua_wrong_passwd'})
    ...
    response_for_me_incorr_token = await test_client.get(
        '/auth/me',
        headers={'Authorization': f'Bearer incorrect_token_sorry'}
    )
